...
| **4** Surveillance | ✅ | baseline→clean→regression cycle, new-defect detection, webhook/file/stdout sinks; `test/surveillance.test.mjs`; on-demand `POST /api/surveillance` **and** a background scheduler (`startSurveillance`) that the server starts when an interval is set (`surveillanceIntervalMin` pref / `CAIRN_SURVEILLANCE_INTERVAL_MIN`). |
| **5** API + AuthZ | ✅ | RBAC (viewer/analyst/admin) + API keys + OIDC hook; `test/auth.test.mjs`; **live-verified** 401/403/200 enforcement; live SSO needs their IdP. The who-did-what **access log** (`auditRecord`) is wired: every gated request is persisted to `.cairn/access.jsonl` and served at `GET /api/access-log`. |
