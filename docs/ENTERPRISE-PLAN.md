...
| **3** Scale (ANN) | ✅ benchmarked | `core/vector-index.mjs` LSH index; **~76× vs. _extrapolated_ brute force** (measured at 2k pairs, scaled ×100 to 20k×768 — the bench prints "extrapolated"; never quote the number without it), 98.3% recall / 100% precision (both exact-measured); wired into `lib/contradict.mjs` with an exact-parity test (`test/contradict-ann.test.mjs`) |
| **4** Surveillance | ✅ on-demand | baseline→clean→regression cycle, new-defect detection, webhook/file/stdout sinks; `test/surveillance.test.mjs`; live `POST /api/surveillance`. *Note:* runs **on demand** per request. `startSurveillance()` (an interval scheduler) is coded but **not** started by the server — no background loop runs yet. |
| **5** API + AuthZ | ✅ | RBAC (viewer/analyst/admin) + API keys + OIDC hook; `test/auth.test.mjs`; **live-verified** 401/403/200 enforcement on the running server; live SSO needs their IdP. *Note:* `auditRecord()` (the who-did-what access log) is coded but **not yet wired** to a sink — the gate enforces access, but per-request access logging is not persisted yet. |
