...
export async function buildIndex(vaultDir, { exts, controlledDirs = [] } = {}) {
  ...
  for (const path of files) {
    ...
    if (DOC_BINARY.has(ext)) {
      ...
      records.push({ rel: drel, name: dname, title: dname, body: dbody, tags: [], outlinks: [], mtimeMs: st.mtimeMs, controlled: ctrlDirs.has(dtop), authority: null, source: 'filesystem' });
      continue;
    }
    let raw;
