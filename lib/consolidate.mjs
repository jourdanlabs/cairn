...
export function extractionPrompt(entity, passages, guidance) {
  const listed = passages.map((p, i) => `[${i + 1}] (source: ${p.note}) ${norm(p.text)}`).join('\n\n');
  const angle = guidance || 'Prefer identity facts (full name, role, relationships), decisions, and current status.';
  return [
    { role: 'system', content: 'You consolidate knowledge from a document archive into verified facts. You output STRICT JSON only — no prose, no code fences. You never invent: every fact must be directly supported by a verbatim quote copied character-for-character from one passage.' },
    { role: 'user', content: `Entity: "${entity}"\n\nPassages:\n${listed}\n\nExtract durable facts about ${entity}. Output exactly this JSON shape:\n{"type":"person|project|company|concept|other","facts":[{"fact":"<short declarative sentence>","quote":"<exact verbatim substring from ONE passage>","passage":<number>}]}\n\nRules: at most 12 facts. The quote must be copied exactly from the passage it cites (it will be machine-checked; paraphrased quotes are discarded). ${angle} Skip speculation, plans that never happened, and anything a quote does not directly support.` },
  ];
}
...
// ── card rendering ──────────────────────────────────────────────────────────────
export function renderCard({ entity, type, verified, dropped, passages, generatedAt, nameFact = null, kind = null }) {
...
// ── orchestration ───────────────────────────────────────────────────────────────
export async function consolidateEntity(index, entity, { chatFn, searchFn, now, guidance, kind } = {}) {
  const passages = gatherPassages(index, entity, { searchFn });
  if (!passages.length) return { entity, kind, passages, verified: [], dropped: [], markdown: null, receipt: null };
  const raw = await chatFn(extractionPrompt(entity, passages, guidance));
  ...
  const { markdown, receipt } = renderCard({ entity, type, verified, dropped, passages, generatedAt, nameFact, kind });
  return { entity, type, kind, passages, verified, dropped, nameFact, markdown, receipt, generatedAt };
}
