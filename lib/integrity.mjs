...
export function integrityReport(index, { vaultName = '', staleDays = 180, contradictionThreshold = 0.82, maxPairs = 25, at = new Date().toISOString() } = {}) {
  ...
  const penalties = {
    ...
    // These are CANDIDATES (same-topic by similarity, may be merely related), so
    // penalize mildly; controlled↔controlled pairs count a little extra as review
    // priorities — but they are NOT confirmed contradictions until adjudicated.
    contradictions: overlaps.available ? Math.min(22, overlaps.pairs.length * 2 + controlledOverlaps) : 0,
  ...
  const report = {
    ...
    contradictions: {
      ...
      note: overlaps.available
        ? 'Same-topic candidates by embedding similarity — NOT confirmed contradictions. controlled_overlaps are authoritative-doc pairs to review first; run adjudication (Overlaps) for contradict/duplicate/related verdicts.'
