...
// Framing for a personal dump that has never been curated. Demo corpora (law/data/
// civic/energy) all carry YAML front-matter, so they do not trip this even though
// they are tag-less — a literal `untagged > 30% OR no schema` would false-positive
// them. AND is the version that keeps both claims true: brain self-labels; boxes
// score as curated.
export const RAW_ARCHIVE_NOTE = 'Raw archive — stub/orphan penalties suppressed; score is not a curation-quality signal.';

export function detectRawArchive(index, counts) {
  const corpus = (index.notes || []).filter((n) => !String(n.rel || '').startsWith('cards/'));
  const n = corpus.length || (index.notes || []).length;
  if (!n) return false;
  const untagged = Number(counts?.untagged ?? corpus.filter((x) => !(x.tags || []).length).length);
  const withFm = corpus.filter((n) => n.hasFrontmatter).length;
  return (untagged / n) > 0.30 && (withFm / n) < 0.70;
}
...
export async function integrityReport(index, {
  ...
  const rawArchive = detectRawArchive(index, a.counts);

  // ---- Penalties ----
  const penalties = {
    broken_links: Math.min(24, a.counts.broken_links * 6),
    stale: Math.min(16, a.counts.stale * 4),
    // Stub notes are never scored. Orphans are scored on a curated corpus and
    // suppressed on a raw archive (the chamber brain is a dump, not a library).
    orphans: rawArchive ? 0 : Math.min(12, a.counts.orphans * 2),
  ...
  const report = {
    ...
    ...(rawArchive ? { archive_note: RAW_ARCHIVE_NOTE, raw_archive: true } : { raw_archive: false }),
    vault: vaultName,
