...
export function search(index, query, { k = 8, perNote = 2, semScores = null } = {}) {
  ...
  // Pass 1: raw BM25 (+ structural boosts) per chunk.
  let maxBm = 0;
  ...
  for (let i = 0; i < index.chunks.length; i++) {
    ...
    scored.push({ c, score: final, matched: matched[i], sem, note: index.byRel?.get(c.noteRel) });
  }
  ...
  if (hybrid) {
    ...
    weak = hits.length === 0 || top < 0.24;
  } else {
    const maxScore = uniq.reduce((s, t) => s + idfs.get(t), 0) * (K1 + 1) / (K1 + 1) || 1;
    confidence = Math.round(Math.max(0, Math.min(1, top / maxScore)) * 100) / 100;
    const coverageTop = hits[0] ? Number(hits[0].coverage.split('/')[0]) / (uniq.length || 1) : 0;
    weak = hits.length === 0 || (confidence < 0.16 && coverageTop < 0.5);
  }
