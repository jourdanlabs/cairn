...
- **Prove it (for regulated environments)** → every answer path (refusal, retrieved passages, grounded synthesis) emits a hashed **receipt** tying it to the exact sources, each content-hashed. A one-click **Integrity Report** scores the whole base (0–100 + grade), rolls up every finding + contradiction, and exports as a signed JSON artifact. Provenance a risk team — or a regulator — can check. The seal binds order, kind, and payload — not wall-clock time; see `docs/COMPLIANCE.md`.
- **Controlled sources (compliance)** → mark authoritative docs (`authority: controlled` in frontmatter, or a top-level folder listed in `CONTROLLED_DIRS`). Answers grounded on a non-controlled source are flagged, and same-topic overlaps between **two controlled documents** are surfaced as **review priorities** — the pairs to adjudicate first, since a genuine conflict between authoritative policies is the costliest kind.
